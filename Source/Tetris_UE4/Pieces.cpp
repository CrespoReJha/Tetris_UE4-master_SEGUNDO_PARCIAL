...
APieces::APieces()
{
    ...
    struct FConstructorStatics
    {
        ...
        FConstructorStatics()
            ...
            , Color_7(TEXT("Material'/Game/StarterContent/Materials/M_Metal_Brushed_Nickel.M_Metal_Brushed_Nickel'")) //material metalico
        {
        }
    };
...
void APieces::SpawnBlocks()
{
    ...
    {
        int color = Index;
        if (getTipoColor() == 1) { // para las piezas coloridas
            color = FMath::RandRange(0, tipoPieza.size() - 1);
        }
        else if (getTipoColor() == 3) { // para las piezas metalicas
            color = 7;
        }

        FRotator Rotation(0.0, 0.0, 0.0);
        ABlock* B = GetWorld()->SpawnActor<ABlock>(this->GetActorLocation(), Rotation);
        B->Mesh->SetMaterial(1, Colors[color]); // asigna el material
        Blocks.Add(B);
