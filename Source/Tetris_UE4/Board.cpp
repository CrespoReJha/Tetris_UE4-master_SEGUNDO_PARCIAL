...
void ABoard::BeginPlay()
{
    ...
    //OBSERVER
    PublicadorScore = GetWorld()->SpawnActor<AOBSERVER_PublicadorScore>(AOBSERVER_PublicadorScore::StaticClass());
    SuscriptorEscenarios = GetWorld()->SpawnActor<AOBSERVER_SuscriptorEscenarios>(AOBSERVER_SuscriptorEscenarios::StaticClass());

    SuscriptorEscenarios->SetPublicadorScore(PublicadorScore);

    //FACADE
    Fachada = GetWorld()->SpawnActor<AFACADE_FachadaBuilder>(AFACADE_FachadaBuilder::StaticClass());
...
void ABoard::Tick(float DeltaTime)
{
    ...
    //OBSERVER
    if (Score >= 1) {
        PublicadorScore->ScoreAlcanzado();
        Score = 0;
    }
    switch(Status)
    {
        ...
        case PS_MOVING:
            ...
            if(CoolLeft <= 0.0f)
            {
                //GEngine->AddOnScreenDebugMessage(-1, 0.5, FColor::Purple, TEXT("Filas eliminadas: ") + FString::FromInt(Score));
                MoveDown();
            }
            break;
        case PS_GOT_BOTTOM:
...
void ABoard::NewPieces()
{
    ...
    //FACADE
    APieces* Pieza = Fachada->ObtenerPiezaBuilder();
    //CurrentPieces = GetWorld()->SpawnActor<APieces>(Location, Rotation);
    CurrentPieces = Pieza;
    //
    
    //STRATEGY
    Estratega = GetWorld()->SpawnActor<ASTRATEGY_Estratega>(ASTRATEGY_Estratega::StaticClass());
    if (Fachada->ObtenerTipoPieza() == 5) {
        EstrategiaLoca = GetWorld()->SpawnActor<ASTRATEGY_EstrategiaLoca>(ASTRATEGY_EstrategiaLoca::StaticClass());
        Estratega->EstablecerEstrategia(EstrategiaLoca);
        Estratega->EstablecerComponente(CurrentPieces);
    }
    else if (Fachada->ObtenerTipoPieza() == 1) {
        EstrategiaLento = GetWorld()->SpawnActor<ASTRATEGY_EstrategiaLento>(ASTRATEGY_EstrategiaLento::StaticClass());
        Estratega->EstablecerEstrategia(EstrategiaLento);
        Estratega->EstablecerComponente(CurrentPieces);
    }
    else if (Fachada->ObtenerTipoPieza() == 2) {
        EstrategiaRapido = GetWorld()->SpawnActor<ASTRATEGY_EstrategiaRapido>(ASTRATEGY_EstrategiaRapido::StaticClass());
        Estratega->EstablecerEstrategia(EstrategiaRapido);
        Estratega->EstablecerComponente(CurrentPieces);
    }
    //
    CurrentPieces->SetActorRelativeLocation(FVector(Location));
    CurrentPieces->SetActorRelativeRotation(FRotator(Rotation));
